...
const bookList= async function (req, res) {
    let listBooks= await BookModel.find().select( { bookName : 1 , authorName: 1 } )
    res.send({msg: listBooks})
}

const getBooksInYear= async function (req, res) {
    let data= req.query.year
    let yearOfBooks= await BookModel.find({year:{$eq: data}})
...
const getXINRBooks=async function(req,res){
    let INROfBooks=await BookModel.find({$or:[{"prices.indianPrice":"100INR"},{"prices.indianPrice":{$eq:"200INR"}},{"prices.indianPrice":{$eq:"500INR"}}]})
        res.send({msg: INROfBooks})
    }
